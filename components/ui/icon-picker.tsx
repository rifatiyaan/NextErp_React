...
import { DynamicIcon, DynamicIconNameType } from '@/components/dynamic-icon';
...
export type IconName = DynamicIconNameType;
...
const IconRenderer = React.memo(({ name }: { name: IconName }) => {
  return <DynamicIcon name={name} className="h-4 w-4" />;
});
...
const useIconsData = () => {
  ...
  useEffect(() => {
    ...
    const loadIcons = async () => {
      ...
      if (isMounted) {
        // Use all icons - DynamicIcon will handle rendering
        // The icon picker should show all available icons from the data
        setIconsList(iconsData);
        setIsLoading(false);
      }
    };
  ...
  return { icons: iconsList, isLoading };
};
...
>(({
  ...
  const renderIcon = useCallback((icon: IconData) => {
    ...
    const iconName = kebabToPascal(icon.name) as IconName;
    return (
  ...
  return (
    <Popover open={open ?? isOpen} onOpenChange={handleOpenChange} modal={modal}>
      <PopoverTrigger ref={ref} asChild {...props}>
        {children || (
          <Button variant="outline">
            {(value || selectedIcon) ? (
              <>
                <DynamicIcon name={(value || selectedIcon)!} className="h-4 w-4 mr-2" /> {value || selectedIcon}
              </>
            ) : (
              triggerPlaceholder
            )}
      ...
      <PopoverContent className="w-64 p-2" align="center" side="bottom" sideOffset={4}>
        {searchable && (
          <Input
